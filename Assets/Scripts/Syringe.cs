...
public class Syringe : Clickable
{
    ...
    void Update()
    {
        if (m_StartListening)
        {
            ...
            {
                m_StartListening = false;
                EventManager.Get().PickedUpSyringe();
    ...
    public override void OnClick()
    {
        if (m_IsPlayerInInteractionRange && !m_WasAlreadyClicked && m_IsPlayerCandleLit)
        {
            ...
            EventManager.Get().ClickedSyringe();
        }
        else if (m_IsPlayerInInteractionRange && !m_IsPlayerCandleLit)
        {
            EventManager.Get().TypeSentenceToHeadBubble("It's too dark here, I need to find a candle to illuminate my surroundings...");
        }
    }
}
