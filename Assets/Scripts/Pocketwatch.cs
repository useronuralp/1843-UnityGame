...
public class Pocketwatch : Clickable
{
    ...
    void Update()
    {
        if (m_StartListening)
        {
            ...
            {
                m_StartListening = false;
                EventManager.Get().PickedUpPocketwatch();
    ...
    public override void OnClick()
    {
        if (m_IsPlayerInInteractionRange && !m_WasAlreadyClicked && m_IsPlayerCandleLit)
        {
            ...
            EventManager.Get().ClickedPocketwatch();
        }
        else if (m_IsPlayerInInteractionRange && !m_IsPlayerCandleLit)
        {
            EventManager.Get().TypeSentenceToHeadBubble("I need light to see it...");
        }
    }
}
