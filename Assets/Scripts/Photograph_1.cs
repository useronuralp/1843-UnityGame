...
public class Photograph_1 : Clickable
{
    ...
    void Update()
    {
        if (m_StartListening)
        {
            ...
            {
                m_StartListening = false;
                EventManager.Get().PickedUpPhotograph();
    ...
    public override void OnClick()
    {
        if (m_IsPlayerInInteractionRange && !m_WasAlreadyClicked && m_IsPlayerCandleLit)
        {
            ...
            EventManager.Get().ClickedPhotograph();
        }
        else if (m_IsPlayerInInteractionRange && !m_IsPlayerCandleLit)
        {
            EventManager.Get().TypeSentenceToHeadBubble("I need a candle to see this photograph...");
        }
    }
}
